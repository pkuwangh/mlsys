...
def dump_bandwidth(
    ...
    if rank == 0:
        print(
            f"{torch.distributed.get_backend(group)} {test_name:<10s}:"
...
def test_gather(group, tensor, data_size, num_iters):
    ...
    if rank == 0:
        dump_bandwidth(
            group, "Gather", data_size, num_iters, elapsed_time, bandwidth, bus_bandwidth
        )
    ...
    for group_info in groups:
        ...
        for test_info in tests:
            for num in sorted(size_and_iters.keys()):
                data = torch.randn(1024, num // test_info["data_size_scale"])
