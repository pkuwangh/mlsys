...
__global__ void matmul_d02_tensor_core_wmma_shmem(bf16 *A, bf16 *B, float *C, int M, int K, int N) {
    ...
    const int num_warp_m = BM / WMMA_M;
    const int num_warp_n = BN / WMMA_N;
    ...
    for (int blk_offset = 0; blk_offset < K; blk_offset += BK) {
        ...
        for (int k = 0; k < BK; k += WMMA_K) {
            nvcuda::wmma::load_matrix_sync(frag_a, As + warp_row * WMMA_M * BK + k, BK);
            nvcuda::wmma::load_matrix_sync(frag_b, Bs + k * BN + warp_col * WMMA_N, BN);
    ...
    // store results
    float* C_warp = C + (warp_row * WMMA_M) * N + (warp_col * WMMA_N);
    nvcuda::wmma::store_matrix_sync(C_warp, frag_c, N, nvcuda::wmma::mem_row_major);
}

void runMatmulD02TensorCoreWmmaShmem(MatmulBuffers &buffers) {
    dim3 blockDim = dim3(D02_BLOCK_SIZE, 1);
    dim3 gridDim = dim3(buffers.N / D02_BLOCK_N, buffers.M / D02_BLOCK_M);

    matmul_d02_tensor_core_wmma_shmem<D02_BLOCK_M, D02_BLOCK_N, D02_BLOCK_K, D02_WMMA_M, D02_WMMA_N, D02_WMMA_K>
        <<<gridDim, blockDim>>>(buffers.dA_bf16, buffers.dB_bf16, buffers.dC, buffers.M, buffers.K, buffers.N);
    // checkCuda(cudaGetLastError(), "launch matmul_d02_tensor_core_wmma_shmem");
    buffers.num_iters += 1;
}
