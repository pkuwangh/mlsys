...
__global__ void matmul_shmem_basic(const float *A, const float *B, float *C, int M, int K, int N) {
...
void runMatmulShmemBasic(MatmulBuffers &buffers) {
    dim3 blockDim = dim3(TILE_SIZE_X1, TILE_SIZE_X1);
    dim3 gridDim = makeGrid2D(buffers.M, buffers.N, blockDim);

    matmul_shmem_basic<<<gridDim, blockDim>>>(buffers.dA, buffers.dB, buffers.dC, buffers.M, buffers.K, buffers.N);
